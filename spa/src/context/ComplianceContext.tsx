...
export type ComplianceContextValue = {
  ...
  accept: () => void;
  reset: () => void;
  setComplianceText: (text: string) => void;
};
...
export function ComplianceProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<ComplianceContextValue>(() => ({ state, setRegion, accept, reset, setComplianceText }), [state, setRegion, accept, reset, setComplianceText]);
